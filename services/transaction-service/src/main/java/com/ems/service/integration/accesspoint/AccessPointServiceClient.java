...
public interface AccessPointServiceClient {
    ...
    @CircuitBreaker(name = "ACCESS-POINT-SERVICE")
    @GetMapping("/api/v1/access-point/nearest")
    ResponseEntity<AppResponse<Optional<AccessPoint>>> getByNearest(@RequestParam("lat") Double lat, @RequestParam("lon") Double lon);

    @CircuitBreaker(name = "ACCESS-POINT-SERVICE")
    @GetMapping("/api/v1/access-point/all")
    ResponseEntity<AppResponse<List<AccessPoint>>> getAll();
